...
//快速排序 n log n
void Q(int s,int t){
	int i=s,j=t;int tmp,m=a[(i+j)>>1];//m=a[s+rand()*rand()%(t-s+1)];
	do {
		while (a[i]<m) ++i;
		while (a[j]>m) --j;
		if (i<=j){
...
void Q_plus(int s,int t){
	...
	int i=s,j=t;int tmp,m=a[s+rand()*rand()%(t-s+1)];//m=a[(s+t)>>1];
	do {
		while (a[i]<m) ++i;
		while (a[j]>m) --j;
		if (i<=j){
...
//快速排序+array n log n
template<class T>
void Qsort(T a[],int s,int t){
	int i=s,j=t;T tmp,m=a[(i+j)>>1];
	do {
		while (a[i]<m) ++i;
		while (a[j]>m) --j;
		if (i<=j){
...
//快速排序+pointer n log n
template<class T>
void Qsort(T *s,T *t){
	T *i=s,*j=t;T tmp,m=*(T*)(s+(t-s)/sizeof(T)/2);
	do {
		while (*i<m) ++i;
		while (*j>m) --j;
		if (i<=j){
...
void Trie_sort(int a[],int l,int r){
	...
	struct print{
		int *&a,&l;
		print(int *&_a,int &_l):a(_a),l(_l){}
		void operator ()(node *x,int key){
			if (!x->son[0]&&!x->son[1]){
				for (int i=1;i<=x->cnt;++i)a[l++]=key;return;
			}
			if (x->son[0])(*this)(x->son[0],key<<1);
			if (x->son[1])(*this)(x->son[1],(key<<1)+1);
		}
	}print(a,l);
	print(root,0);
	#undef W
...
//离散
#include<map>
void disperse(int a[],int l,int r){
	map<int,int> M;
	M.clear();for (int i=l;i<=r;++i)M[a[i]]=0;int s=0;
	for (map<int,int>::iterator zzy=M.begin();zzy!=M.end();++zzy)zzy->second=++s;
	for (int i=l;i<=r;++i)a[i]=M[a[i]];
}
int main()
{
	...
	init(2650000);t1=clock(); Qsort(a+1,a+n);
	printf("Q_pointer:time=%d\n",clock()-t1);check();
	...
	init(440000);t1=clock(); Trie_sort(a,1,n);
